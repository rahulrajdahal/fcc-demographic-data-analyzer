...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round((df[df['hours-per-week'] == min_work_hours]['salary'] == '>50K').sum() / num_min_workers * 100,1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = df[df['salary'] == '>50K']['native-country'].value_counts().idxmax()
    highest_earning_country_percentage = round((df[df['native-country'] == highest_earning_country]['salary'] == '>50K').sum() / df[df['native-country'] == highest_earning_country].shape[0] * 100,1)
